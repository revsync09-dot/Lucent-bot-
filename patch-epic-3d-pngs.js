...
function replaceFunction(funcName, newCode) {
  ...
  const idx = code.ind0exOf(startText);
  if (idx === -1) {
...
function drawEpic3DBox(ctx, x, y, w, h, baseColor, glowColor = null, angleRight = 0) {
...
const newProfile = \`async function generateProfileCard(user, hunter) {
  ...
  let safeName = ellipsizeText(ctx, user.username || user.displayName || "Unknown", 350);
  ctx.shadowColor = "#FFF";
  ctx.shadowBlur = 10;
  ctx.fillText(safeName, cx, cy + 160);
  ctx.shadowBlur = 0;

  // Level & Rank badge floating in 3D
  ctx.font = "700 24px Rajdhani";
  ctx.fillStyle = "#94A3B8";
  ctx.fillText(\`Level \${hunter.level} | \${hunter.rank}\`, cx, cy + 200);
  
  // Class Label
  const hClass = String(require("./classService").getHunterClass(hunter)).toUpperCase();
  drawEpic3DBox(ctx, cx - 120, cy + 240, 240, 50, "#8B5CF6");
  ctx.fillStyle = "#FFFFFF";
  ctx.font = "900 28px Orbitron";
  ctx.fillText(\`\${hClass}\`, cx, cy + 276);

  // Exp progression bar below class
  const maxExp = Math.ceil(100 * Math.pow(hunter.level, 1.5));
  const expPercent = Math.min(Math.max((hunter.exp||0) / maxExp, 0), 1);
  ...
  ctx.fillText(\`\${hunter.exp} / \${maxExp} XP\`, cx, cy + 338);

  // Right Side: Beautiful Data Arrays
  // Upper Right: Stats
  ...
  for(let i=0; i<stats.length; i++) {
     ...
     ctx.fillText(Number(s.val), bx + 255, by + 48);
     ctx.shadowBlur = 0;
     ctx.textAlign = "left";
  }

  // Lower Right: Resources & Guild
  drawEpic3DBox(ctx, 480, 400, 680, 310, "rgba(245, 158, 11, 0.8)");
  ctx.fillStyle = "#F59E0B";
  ctx.font = "900 34px Orbitron";
  ctx.fillText("RESOURCES", 520, 450);
...
const newHunt = \`async function generateHuntResultCard(user, result, levelsGained) {
  ...
  // Red/Dark background indicating battle aftermath
  ...
  ctx.fillText(\`Target Eliminated by \${user.username}\`, width/2, 140);
  
  // 3D Loot Boxes
  drawEpic3DBox(ctx, 150, 200, 250, 140, "#FBBF24");  // Gold box
  drawEpic3DBox(ctx, 500, 200, 250, 140, "#3B82F6");  // XP Box
  ...
  ctx.fillText(\`+\${result.gold}\`, 275, 270);
  ctx.shadowBlur = 0;
  ...
  ctx.fillStyle = "#3B82F6";
  ctx.font = "900 45px Orbitron";
  ctx.fillText(\`+\${result.xp}\`, 625, 270);
  ctx.shadowBlur = 0;
  ctx.font = "700 22px Rajdhani";
  ctx.fillStyle = "#FFF";
...
const newGate = \`async function generateGateCard(user, difficultyText, results, isSuccess) {
  ...
  ctx.fillStyle = "#FFF";
  ctx.font = "700 28px Rajdhani";
  ctx.fillText(\`Difficulty: \${difficultyText}\`, width/2, 150);
  ...
  if (isSuccess) {
      ...
      ctx.fillText(\`+\${results.xp} XP  |  +\${results.gold} GOLD\`, width/2, 275);
      ctx.shadowBlur = 0;
  } else {
      ...
      ctx.fillText(\`PENALTY: -\${results.penalty} GOLD\`, width/2, 270);
      ctx.shadowBlur = 0;
  }
...
console.log("Successfully rewrote PNG rendering with EPIC 3D UI!");