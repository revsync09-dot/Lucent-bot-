...
if (!code.includes("drawEpic3DBox")) {
  ...
  code = code.slice(0, lastConst) + helper3D + "\n" + code.slice(lastConst);
}

function safeReplace(funcName, newFuncNameStr) {
  ...
  // Find the END of the function body. The next 'async function' or 'module.exports' is usually safe.
  ...
  code = code.slice(0, start) + newFuncNameStr + "\n" + code.slice(nextFunc);
...
safeReplace("generateGateCard", newGate);

fs.writeFileSync(path.join(__dirname, "src/services/cardGenerator.js"), code, "utf8");
console.log("SUCCESS! All cards rewritten precisely.");